...
int main(void){
	...
	std::function<float(const float&)> 	f 	= func_f;
	
	rootSolve_bisect               (     f,xL,xR,x);
	rootSolve_bisect<float ,float >(     f,xL,xR,x);
	//rootSolve_bisect<double,double>(     f,xL,xR,x);	// does not work: cannot convert float to double
	rootSolve_bisect<float ,float >(func_f,xL,xR,x);
	//rootSolve_bisect<double,double>(func_f,xL,xR,x);	// does not work: cannot convert float to double
	...
	rootSolve_newton<float ,float >(     f,x);
	//rootSolve_newton<double,double>(     f,x);
	rootSolve_newton<float ,float >(func_f,x);
